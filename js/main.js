...
jQuery(function($){
    $('#submit-copymatic-api-key').on('click', function(){
		...
		if(api_key!=''){
			$.ajax({
				...
				dataType: "json",
				url: admin_url,
				data: {action: "check_copymatic_api", apikey: api_key, _ajax_nonce: copymatic_ajax_object.nonce},
	...
	$(document).on('click', '.delete_article', function(e){
		...
		$.ajax({
			...
			dataType : "json",
			url : admin_url,
			data : {action: "copymatic_delete_article", id: id, _ajax_nonce: copymatic_ajax_object.nonce},
			success: function(r) {
	...
	$(document).on('click', '.import_article', function(e) {
		...
		$.ajax({
			type: "post",
			dataType: "json",
			url: copymatic_ajax_object.ajax_url,
			data: {action: "copymatic_import_article", id: id, _ajax_nonce: copymatic_ajax_object.nonce},
			success: function(r) {
				if (typeof r.success !== 'undefined' && r.success == true) {
					var json = r.data;
					if (json.post_id) {
						_this.remove();
						var posts_admin_url = copymatic_ajax_object.posts_admin_url.replace('admin-ajax.php', '');
	...
	$(document).on('click', '.import_generated_article', function(e){
		...
		$.ajax({
			...
			dataType : "json",
			url : admin_url,
			data : {action: "copymatic_import_generated_article", id: id, title: title, _ajax_nonce: copymatic_ajax_object.nonce},
			success: function(r) {
				if(typeof r.success !== 'undefined' && r.success == true){
					var json = r.data;
					if(json.post_id){
	...
	function generate_ideas(model){
		...
		var request_data = {
            ...
            language:language,
			creativity:"regular",
			_ajax_nonce: copymatic_ajax_object.nonce
        }
